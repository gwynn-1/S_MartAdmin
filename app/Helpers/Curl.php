...
class Curl
{
    ...
    /**
     * Execute call api and return result
     *
     * @param string $url
     * @return mixed
     */
    public function execute($url)
    {
        ...
        if ($this->method == self::METHOD_POST) {
            ...
            //curl_setopt($this->ch, CURLOPT_POSTFIELDS, json_encode($this->dataPost ?: []));
            curl_setopt($this->ch, CURLOPT_POSTFIELDS, is_array($this->dataPost) ? $this->getPostVal($this->dataPost) : $this->dataPost);

        }

        $response = curl_exec($this->ch);

        if (! $this->disHeaderRes && $response)
        {
            $this->setHeaderRes($this->ch, $response);
        }

        return !$this->getError() ? json_decode($response, true) : false;
    }
    ...
    public function getPostVal($arrData)
    {
        $postvars = array();
